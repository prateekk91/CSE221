...
void memory_latency(unsigned long size, unsigned long iters, unsigned long stride_size, FILE* file) 
{
	...
	for (k = stride_size; k < size; k+=stride_size) 
	{
		*(char **)&array[k - stride_size] = (char*)&array[k];	
	}
	*(char **)&array[k - stride_size] = (char*)&array[0];	
	p = array;
	...
	time_taken = (end - start) / (iters*count) * 2.592711812; //The time taken is the time spent on a single read.
...
{
	warmUp();
	unsigned long iters = 1000000; // 2 Million iterations.
